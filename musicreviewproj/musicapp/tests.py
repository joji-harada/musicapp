...
class MusicTypeTest(TestCase):
    def test_string(self):
        type=MusicType(typename="Instruments")
...
class ProductTest(TestCase):
    ...
    def setup(self):
        type = MusicType(typename='Instruments')
        product=Product(productname='Ibanez', producttype=type, productprice='435.00')
        return product
    def test_string(self):
        prod = self.setup()
    ...
    def test_discount(self):
        prod=self.setup()
        self.assertEqual(prod.memberdiscount(), 21.75)
    def test_type(self):
        prod=self.setup()
        self.assertEqual(str(prod.producttype), 'Instruments')
...
class ReviewTest(TestCase):
   def test_string(self):
       rev=Review(reviewtitle="SM58 Review")
...
class GetProductsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.type=MusicType.objects.create(typename='Instruments')
        ...
        self.rev1=Review.objects.create(reviewtitle='productreview', reviewdate='2019-04-03', product=self.prod, reviewrating=4, reviewtext='some review')
        self.rev1.user.add(self.u)
        self.rev2=Review.objects.create(reviewtitle='productreview', reviewdate='2019-04-03', product=self.prod,  reviewrating=4, reviewtext='some review')
...
class New_Product_authentication_test(TestCase):
    def setUp(self):
        ...
        self.type=MusicType.objects.create(typename='laptop')
    ...
    def test_redirect_if_not_logged_in(self):
        ...
        self.assertRedirects(response, '/accounts/login/?next=/musicapp/newproduct/')
    ...
    def test_Logged_in_uses_correct_template(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'musicapp/newproduct.html')